...
document.querySelector(".talk-icon").addEventListener("click", async () => {
  ...
  try {
    ...
    window.location.href = `./chat_room.html?display=${urlParams.get(
      "nickname"
    )}&room=${response.name}`;
  } catch (e) {
    console.log(e);
  ...
  if (user) {
    ...
    const contentPost = async () => {
      const result = await fetch(
        `${baseurl}` + `api/v1/account/profile/?user=${queryParams.nickname}`,
        {
          method: "GET",
        ...
        .then((res) => {
          ...
          function verfiyProfile() {
            ...
            if (personalInfo.nickname != res.user.nickname) {
              ...
              document
                .querySelector(".tab-menu-profile")
                .classList.remove("on");
            } else {
          ...
          verfiyProfile();
          let followListBtn = document.querySelectorAll(
            ".main-profile-follow a"
          );
          ...
          document.querySelector(".followers").innerHTML =
            res.user.followers_length;
          document.querySelector(".followings").innerHTML =
            res.user.followings_length;
          document
            .querySelector(".profile-img img")
            .setAttribute(
              "src",
              res.user.image_url == null || res.user.image_url == ""
                ? "../img/peach_cha.png"
                : res.user.image_url
            );
          document.querySelector(".user-nickname").innerHTML =
            res.user.nickname;
          ...
          if (res.post) {
            ...
            res.post.forEach((post) => {
              ...
              document.querySelector(
                ".post-userinfo-img"
              ).innerHTML = `<img src="${user_img}" alt="user-img">`;
